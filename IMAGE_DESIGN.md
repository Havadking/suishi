...
> 状态：v1.4 已实现（Sprint A / B / C 全部落地）。本文保留为设计依据；实现与文中的差异见文末「实现备注」。前端可点击原型见 [prototypes/image-browser.html](prototypes/image-browser.html)（在线版：https://claude.ai/artifact/4SpPknJEiUwBBjB4ATiGo9 ，右下角「导览」可跳到 5 个场景，勾选「显示设计标注」看设计说明）。
...
- **滚轮语义**（§3.7 原设计"滚轮缩放"）：改为 **滚轮翻上一张 / 下一张**，`Ctrl`/`⌘` + 滚轮缩放；图片在舞台里纵向放不下时（竖长截图、放大后）滚轮改为上下平移。触控板连发的小 delta 累计到阈值再翻，并有 280ms 冷却。
- **单击图片关闭查看器**：没有拖动距离的单击延迟 260ms 后关闭，给双击缩放让路。
- **幻灯放映途中在视频上按空格**：同时停止放映并暂停视频（原设计只规定了图片上的 Space）。
- **翻页跳过不可解码项**：上一张 / 下一张会越过 MKV / HEIC 等条目，箭头可见性也按"方向上是否还有可打开项"计算；Feed 同样只带可打开的条目。
- **图片载入**：`img.decode()` 在页面处于后台时可能一直不结算，所以改为 `load` 事件 + 1.5s 限时 decode，超时直接显示。
- **像素尺寸解析**读文件头前 256KB（原设计 64KB），换取 APP1 较大的手机照片也能在同一次读取里拿到 SOF。
- **「按拍摄时间」**：切换时一次性读完未读 JPEG 文件头再整体重排一次，缩略图缓存命中时 EXIF 直接从缓存回填，不再读文件。
- **Feed 里的静音按钮**在图片上置灰而非隐藏，避免 HUD 布局跳动。
- **预载相邻图片**（§3.7）未做：`File` 句柄本身很轻，翻页耗时主要在 decode，实测无明显收益。
- **IDB 版本**保持 4；`thumbnails` 表的值多了 `w / h / exif / exifRead`，`kv` 多了 `mediaMode:<folderId>` 与 `imageLayout:<folderId>`。
- `savedFolders[i]` 新增 `videoCount / imageCount`，旧数据只有 `count` 时侧栏显示「N 个文件」，重新扫描一次即更新。

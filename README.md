...
> 纯前端、零依赖、单文件架构的本地短视频与图片浏览工具。依托现代 Chromium 浏览器的原生 Web API，采用温馨亲切的“日常亲切风”（Warm Cozy Aesthetic）设计，配备可伸缩多目录持久化管理侧边栏，提供传统媒体库网格浏览、图片查看器，以及类似抖音 / TikTok 的沉浸式垂直滑动无缝播放体验（v1.4 起视频与图片同一抽屉混排）。

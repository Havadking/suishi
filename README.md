...
> 纯前端、零依赖、单文件架构的本地短视频与图片浏览工具（v1.5 起附带一个可选的零依赖 Node 局域网分享服务，让平板 / 手机也能看电脑里的视频）。依托现代 Chromium 浏览器的原生 Web API，采用温馨亲切的“日常亲切风”（Warm Cozy Aesthetic）设计，配备可伸缩多目录持久化管理侧边栏，提供传统媒体库网格浏览、图片查看器，以及类似抖音 / TikTok 的沉浸式垂直滑动无缝播放体验（v1.4 起视频与图片同一抽屉混排）。
...
- [4. 核心功能与交互机制剖析](#4-核心功能与交互机制剖析)
  ...
  - [4.8 「⬇️ 下载」页与拾光笺联动（可选）](#48-下载页与拾光笺联动可选)
  - [4.9 「🖼 看图」：图片与视频同一抽屉](#49-看图图片与视频同一抽屉)
  - [4.10 「📡 局域网模式」：平板 / 手机看电脑里的视频](#410-局域网模式平板--手机看电脑里的视频可选v15)
...
- **100% 离线隐私安全**：基于现代浏览器沙箱本地读取文件，随拾自身不产生任何外发网络流量。可选的「⬇️ 下载」页只与同一台机器上的拾光笺服务（`127.0.0.1`）通信，且只在你主动贴入链接时由拾光笺去访问对应站点。
...
- **格式过滤器**：
  ...
  - `IMAGE_EXT`（支持识别列表）：`jpg`, `jpeg`, `png`, `gif`, `webp`, `avif`, `bmp`, `svg`, `ico`, `heic`, `heif`, `tif`, `tiff`, `psd`, `raw`, `cr2`, `cr3`, `nef`, `arw`, `dng`
  - `NATIVE_VIEWABLE`（原生可解码查看）：`jpg`, `jpeg`, `png`, `gif`, `webp`, `avif`, `bmp`, `svg`, `ico`
  - 每个条目带 `kind: 'video' | 'image'`；不在原生列表里的格式（MKV、HEIC、PSD…）只识别不解码，点击弹出拦截提示。
- **动态进度反馈**：扫描过程中每发现 25 个文件在顶部状态条实时更新计数，避免大目录用户感知卡顿。
...
- **中文拼音自然排序**：通过 JavaScript 国际化 API `a.name.localeCompare(b.name, 'zh')`，原生支持中文拼音字母序与数字自然排序，并支持按修改时间、文件体积、拍摄时间（EXIF，无则回退修改时间）排序以及一键切换升序/降序（↑/↓）。

### 4.5 媒体库自适应网格卡片系统
- **CSS Grid 自适应**：`repeat(auto-fill, minmax(230px, 1fr))`，随窗口缩放自适应列数；「仅图片」模式可切换为 CSS 多列瀑布流，按原始比例排布。
- **一套卡片，两种媒体**：位置完全对齐，只换角标语义——视频右下是时长、图片右下是像素尺寸；左上角标深灰 = 视频、鼠尾草绿 = 图片、红 = 需外部打开；GIF 另有「动图」角标。
- **悬停快捷入口**：视频卡片浮现 `▶`（以此为起点进入刷视频流），图片卡片浮现 `🔍`（打开查看器）。
...
- 居中模态对话框，支持标准全屏、音量、进度拖拽原生控件，支持 PageUp / PageDown（或 N / P）无缝切换上下一个文件；上下一个可以是视频也可以是图片，外壳共用，舞台按类型切换。

### 4.8 「⬇️ 下载」页与拾光笺联动（可选）
随拾不自带后端。刷到怕被删的 B 站 / 抖音视频时，把链接贴进侧栏的「⬇️ 下载」页，由同一台电脑上的 [拾光笺 · Glean](../video-summarizer)（`vsum ui`，默认 `127.0.0.1:7860`）用 yt-dlp 合流成 mp4 存进它配置的 `download.video_dir`，旁边写一个 `<同名>.suishi.json` 记录原链接、作者、完整标题。

- **贴链接 → 探测 → 下载**：支持整段抖音分享口令；探测卡显示封面、作者、时长、可选清晰度上限，以及"已下载过 / 拾光笺里已有转写"的提示；同一视频不会重复下载。
- **进度**：一条 SSE 看全部任务（排队 / 下载中 / 合流 / 完成 / 失败），侧栏入口有连接状态圆点和进行中计数，去别处刷视频也能回来看。
- **目录绑定**：把拾光笺的下载目录加进抽屉并绑定一次，之后下载完成会自动增量刷新；「▶ 播放」直接在随拾里打开刚存好的视频。
- **来源联动**：目录遍历时顺手识别 `.suishi.json`（不多读一次目录），卡片右下角出现来源徽标（🅱 B 站 / 🎵 抖音），播放器和 Feed 多出「🔗 原链接」「📝 拾光笺」按钮（快捷键 `G`）：拾光笺里已有转写就跳过去看，没有就一键发过去排队转写。删除视频时 sidecar 一并删除。
- **降级**：拾光笺没启动时该页显示引导卡并每 10 秒探活；其余功能完全不受影响。连接地址可在页内「⚙ 连接设置」修改，只接受本机地址。

完整设计见 [DOWNLOAD_DESIGN.md](DOWNLOAD_DESIGN.md)。

### 4.9 「🖼 看图」：图片与视频同一抽屉
手机相册备份、旅行照片、聊天截图和 Vlog 素材本来就混在同一个目录里。v1.4 起扫描时把图片一并收进抽屉，与视频共用目录、分组、搜索、排序、星标、重命名、删除和缩略图缓存。

- **媒体类型分段器**：顶栏搜索框旁「全部 / 🎬 视频 / 🖼 图片」，和搜索一样是对当前抽屉的过滤；选择按目录记忆（IDB `kv`），目录里没有该类型时自动回到「全部」。侧栏副标题、胶囊栏、分组计数、统计栏都改为「N 视频 · M 图片」。
- **缩略图**：`createImageBitmap(file, { resizeWidth: 320, imageOrientation: 'from-image' })` 一步完成缩放与 EXIF 方向转正，绘到 320×180 后与视频海报写进同一个 `thumbnails` 表；4 路解码并发闸门，一屏几十张照片同时进视口也不会撑爆主线程。像素尺寸只读文件头前 256KB（JPEG SOF / PNG IHDR / GIF / WebP / BMP）解析，不做完整解码。瀑布流用另一份保留原始比例的 `natural` 变体，分开缓存。
- **图片查看器**：与视频共用 `#modal` 外壳和 ⭐ ✏️ 🗑️ ⛶ ✕ 一套按钮，新增「ℹ 信息」。滚轮翻上一张 / 下一张，`Ctrl` + 滚轮以光标为不动点缩放（`fit×0.5` ～ `8×`），放大后拖拽平移，双击在「适应」与 2× 间切换，单击图片关闭；高宽比超过 2.5 的竖长截图默认「适应宽度」，滚轮上下平移。`R` 旋转 90°、`0` 适应、`1` 原始像素、`I` 信息面板、`Space` 幻灯放映（图片每张 4 秒，遇到视频等它播完再走，到末尾自动停止）。底部缩略图条展示当前位置 ±8 张。超过 40 MP 的图先缩到 4096px 再显示。
- **EXIF**：手写 TIFF/IFD 解析（只读 JPEG APP1，约 60 行），取相机、镜头、光圈、快门、ISO、焦距、拍摄时间、方向；**不读 GPS**。结果随缩略图一起进 IDB，信息面板显示；「按拍摄时间」排序切换时一次性把未读过的 JPEG 文件头读完再整体重排一次，不在浏览途中逐张挪卡片。
- **Feed 里的图片**：作为定时停留 5 秒的一帧混入刷视频流——同图高斯模糊铺底（竖图两侧不再死黑）、顶部细进度条，点一下暂停 / 继续，滚轮或触摸立即取消自动前进。只有浏览器能打开的条目会进 Feed。
- **不可解码格式**：HEIC / RAW / PSD 与 MKV 一样只识别不解码，点击弹出拦截提示并可复制相对路径。
...
File System Access API 只能读运行浏览器那台设备自己的硬盘，平板打开页面拿不到电脑上的文件。v1.5 加了一个**可选的伴生小服务** [`server.mjs`](server.mjs)（零依赖，只用 Node 内置模块），在电脑上把若干目录以 HTTP 方式共享给同一 Wi-Fi 下的设备。电脑端原有的 `file://` / `npx serve` 用法完全不变——页面只有从这个服务同源加载时才进入局域网模式。

```bash
# 电脑上：共享一个或多个目录，默认端口 8970（第一次运行 Windows 防火墙会问一次，选「允许」）
node server.mjs D:\Videos E:\Clips
# 可选：口令 / 端口 / 关掉服务端封面
node server.mjs --token 1234 --port 9000 D:\Videos
# 不带参数时读取同目录的 share.json（模板见 share.example.json）
npm run share
```
启动后终端会打印 `http://192.168.x.x:8970/`，平板 / 手机浏览器打开即可。

- **前端怎么分流**：启动时探测同源 `/api/share/info`，拿到 JSON 才进入局域网模式（`npx serve` 返回 404，`file://` 根本不发请求，所以桌面端不受影响）。条目结构与本地一致，只是没有 `handle`、多了 `remote` / `folderId`；`getMediaBlob` / `getMediaHead` / `getMediaSrc` 三个适配函数把「取文件」统一起来，缩略图、Feed、播放器、图片查看器、EXIF、sidecar 读取都不区分来源，`<video>` 直接吃 `/media/...` 的 URL。
...
- **口令**：`--token` 启用后所有接口需要 Cookie（`HttpOnly`，30 天），页面上会出现口令输入框；不设口令则同一局域网内任何设备可看。
- **只读**：不提供删除 / 重命名接口，平板上相应按钮隐藏，快捷键触发时提示「请在电脑上操作」。星标、断点续播、缩略图缓存存在平板自己的 IndexedDB 里，目录 id 由绝对路径哈希得出，服务重启后仍对得上。
- **触屏与窄屏**：`<video playsinline>`；`(hover: none)` 下悬停才出现的星标 / 刷视频按钮常显；宽度 ≤ 900px 时侧栏变成从左滑出的抽屉（顶栏 ☰ 打开，点遮罩收起），网格列宽收紧，播放器控制条换行。
- **格式仍受平板浏览器限制**：iPad 基本只播 MP4 / MOV（H.264 / HEVC），MKV 与浏览器解不了的编码依然走拦截提示；本项目不做转码。
...
| **点击侧栏「⬇️ 下载」** | 打开下载页；粘贴链接（Ctrl + V）自动探测，Enter 探测 |
| **播放器 / Feed 中 G** | 发送到拾光笺转写，或打开已有的转写（仅对带 `.suishi.json` 的视频生效） |
| **顶栏「全部 / 🎬 视频 / 🖼 图片」** | 只看某一种媒体；「图片」模式下多出「▦ 瀑布流」切换 |

### 网格媒体库
| 操作 | 功能说明 |
| :--- | :--- |
| **单击卡片** | 视频打开弹窗播放器，图片打开查看器；MKV / HEIC 等不可解码格式弹出提示 |
| **卡片悬停点击 ▶ / 🔍** | 视频：以该视频为第一项进入刷视频流；图片：打开查看器 |
| **点击顶部「🔀 刷一刷 / 刷视频 / 刷图片」** | 从列表首项开启 Feed（勾选「随机顺序」则乱序），文案跟随当前媒体模式 |
...
| **图片幻灯片：单击画面** | 暂停 / 继续 5 秒停留计时（滚轮或触摸滑动也会取消自动前进） |

### 图片查看器
| 快捷键 / 手势 | 功能说明 |
| :--- | :--- |
| **← / → 、PageUp / PageDown、N / P、滚轮** | 上一张 / 下一张（跨视频与图片，自动跳过不可解码项；视频播放器里滚轮同样翻上下一个） |
| **Ctrl + 滚轮 / + / −** | 以光标为不动点缩放（工具条 ＋ − 以画面中心缩放） |
| **拖拽 / 双击** | 放大后拖拽平移；双击在「适应」与 2× 之间切换 |
| **单击画面 / Esc / ✕** | 关闭查看器，网格自动滚到刚才看的那一项并短暂高亮（退出 Feed 同样如此） |
| **0 / 1 / R** | 适应窗口 / 原始像素 / 顺时针旋转 90° |
| **I** | 显示 / 隐藏信息面板（文件 + EXIF） |
| **Space / ▶ 幻灯** | 幻灯放映：图片每张 4 秒，视频播完再走，末尾自动停止；在视频上按空格 = 停止放映 |
...
平板 / 手机在同一局域网访问，见 [4.10 局域网模式](#410-局域网模式平板--手机看电脑里的视频可选v15)：`node server.mjs <目录>`，然后在平板上打开终端里打印的地址。
